...
if (reduxStore[key].encrypt) {
  ...
  return {
    data: CryptoJS.AES.encrypt(JSON.stringify(inboundState), r).toString(),
...
(outboundState, key) => {
  if (reduxStore[key].version && (!outboundState.version || outboundState.version !== reduxStore[key].version)) {
    ...
    return Immutable(JSON.parse(bytes.toString(CryptoJS.enc.Utf8)))
  }
  return Object.assign(reduxStore[key].redux.INITIAL_STATE, outboundState)
},
